...
st.set_page_config(page_title="Facade Wind Pressure Analyzer", page_icon="🏗️", layout="wide")
...
def geocode_city(city_name):
    try:
        geolocator = Nominatim(user_agent=f"facade_analyzer_{int(time.time())}", timeout=10)
        time.sleep(1)
...
def get_spec_status(exceedances_per_year):
    if exceedances_per_year == 0:
        ...
        return "Borderline", "orange", f"Exceeded ~{round(exceedances_per_year, 1)}x per year — review your spec carefully."
    else:
        return "Upgrade Needed", "red", f"Exceeded ~{round(exceedances_per_year, 1)}x per year — this spec is insufficient."

with st.sidebar:
    st.header("Analysis Parameters")
    city_name = st.text_input("City / Location", value="Halifax, Canada")
    col1, col2 = st.columns(2)
    with col1:
        start_date = st.date_input("Start Date", value=date.today() - timedelta(days=365 * 10), max_value=date.today() - timedelta(days=5))
    with col2:
    ...
    st.caption("- **137 Pa** — Windows (ASTM E331)\n- **300 Pa** — Curtain Wall (AAMA 501)\n- **480 Pa** — Masonry / Cladding (ASTM E514)")
    analyze_btn = st.button("Analyze Location", use_container_width=True, type="primary")

st.title("Facade Wind Pressure Analyzer")
st.markdown("A **design decision engine** for facade engineers and architects. Enter a city and date range to understand real wind pressure demands — backed by historical weather data, not just code estimates.")
st.divider()
...
if analyze_btn:
    ...
    st.success(f"Location: {display_address} ({lat:.4f}N, {lon:.4f}E) — {years_of_data:.1f} years of data")
    ...
    c1.metric("All-Time Peak Pressure", f"{peak_pa:.0f} Pa", f"{peak_gust:.0f} km/h gust")
    c2.metric("99th Percentile Pressure", f"{p99_pa:.0f} Pa")
    c3.metric("Average Hourly Pressure", f"{avg_pa:.0f} Pa")
    ...
    st.caption("Average gust pressure by month and year. Red = dangerous months.")
    ...
    fig_heat = go.Figure(data=go.Heatmap(
        ...
        colorscale="RdYlBu_r", colorbar=dict(title="Avg Pa"),
        hovertemplate="Year: %{y}<br>Month: %{x}<br>Avg: %{z:.0f} Pa<extra></extra>",
    ))
    fig_heat.update_layout(xaxis_title="Month", yaxis_title="Year", height=350, margin=dict(t=20, b=40), plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
    st.plotly_chart(fig_heat, use_container_width=True)

    st.subheader("Average Pressure by Month")
    st.caption("Which months consistently demand the most from your facade.")
    ...
    fig_bar.add_trace(go.Bar(
        x=monthly_overall["month_name"], y=monthly_overall["gust_pressure_pa"],
        marker_color=["#F44336" if p >= 300 else "#FF9800" if p >= 137 else "#2196F3" for p in monthly_overall["gust_pressure_pa"]],
        hovertemplate="<b>%{x}</b><br>Avg: %{y:.0f} Pa<extra></extra>",
    ))
    for name, vals in THRESHOLDS.items():
        fig_bar.add_hline(y=vals["pa"], line_dash=vals["dash"], line_color=vals["color"], line_width=1.5,
            annotation_text=f"{name.split('(')[0].strip()} {vals['pa']} Pa", annotation_position="right",
            annotation_font_color=vals["color"], annotation_font_size=11)
    fig_bar.update_layout(xaxis_title="Month", yaxis_title="Avg Gust Pressure (Pa)", height=380,
        margin=dict(t=20, b=40, r=220), plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        yaxis=dict(gridcolor="rgba(128,128,128,0.15)"), showlegend=False)
    st.plotly_chart(fig_bar, use_container_width=True)

    st.subheader("Pressure Exceedance Curve")
    st.caption("How many hours per year does pressure exceed a given level?")
    ...
    fig_exc = go.Figure()
    fig_exc.add_trace(go.Scatter(x=pressure_levels, y=exceedance_hours, mode="lines",
        line=dict(color="#1565C0", width=2.5), fill="tozeroy", fillcolor="rgba(21,101,192,0.1)",
        hovertemplate="Pressure: %{x:.0f} Pa<br>Exceeded: %{y:.1f} hrs/year<extra></extra>"))
    for name, vals in THRESHOLDS.items():
        hrs = (df["gust_pressure_pa"] >= vals["pa"]).sum() / years_of_data
        fig_exc.add_vline(x=vals["pa"], line_dash=vals["dash"], line_color=vals["color"], line_width=2,
            annotation_text=f"{vals['pa']} Pa -> {hrs:.1f} hrs/yr", annotation_position="top right",
            annotation_font_color=vals["color"], annotation_font_size=11)
    fig_exc.update_layout(xaxis_title="Wind Pressure (Pa)", yaxis_title="Hours Exceeded Per Year",
        height=400, margin=dict(t=20, b=40), plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        yaxis=dict(gridcolor="rgba(128,128,128,0.15)"), xaxis=dict(gridcolor="rgba(128,128,128,0.15)"), showlegend=False)
    st.plotly_chart(fig_exc, use_container_width=True)

    st.divider()

    st.subheader("Top 10 Worst Recorded Storm Events")
    ...
    st.markdown(f"**Location:** {city_name} | **Period:** {start_date} to {end_date} ({years_of_data:.1f} years) | **Peak month:** {worst_month} | **99th percentile:** {p99_pa:.0f} Pa")

    color_map = {"green": "#E8F5E9", "orange": "#FFF3E0", "red": "#FFEBEE"}
    border_map = {"green": "#4CAF50", "orange": "#FF9800", "red": "#F44336"}
    emoji_map = {"green": "✅", "orange": "⚠️", "red": "🚨"}
    ...
    for name, vals in THRESHOLDS.items():
        ...
        st.markdown(
            f'<div style="background-color:{color_map[color]};border-left:5px solid {border_map[color]};border-radius:6px;padding:14px 18px;margin-bottom:12px;">'
            f'<div style="font-size:1.05em;font-weight:700;margin-bottom:4px;">{name} — {vals["pa"]} Pa</div>'
            f'<div style="font-size:1.15em;font-weight:800;">{emoji_map[color]} {status}</div>'
            f'<div style="margin-top:6px;color:#444;font-size:0.9em;">Exceeded <b>{per_year:.1f}x per year</b> | Return period: <b>{rp_str}</b><br>{note}</div>'
            f'</div>',
    ...
    with st.expander("Download Full Dataset (CSV)"):
        ...
        st.download_button(label="Download CSV", data=csv_out.to_csv().encode("utf-8"),
            file_name=f"facade_wind_{city_name.replace(' ','_')}_{start_date}_{end_date}.csv", mime="text/csv")
    ...
    **Tip:** Use a 10-year date range for the most reliable results.
    """)
